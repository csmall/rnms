...
def poll_tcp_status(poller_buffer, **kwargs):
    ...
    """
    port = str(kwargs['parsed_params'])
    return kwargs['pobj'].tcp_client.get_tcp(kwargs['attribute'].host, port, '', 1, cb_tcp_status, **kwargs)
...
def cb_tcp_status(host, response, connect_time, error, kwargs):
    ...
    if error is None:
        ...
        state = 'closed'
        tcp_error = error[1]
    kwargs['pobj'].poller_callback(kwargs['attribute'], [state, response, connect_time, tcp_error])
