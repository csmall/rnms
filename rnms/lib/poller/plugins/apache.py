...
return_fields = ('Total Accesses', 'Total kBytes', 'CPULoad', 'Uptime',
                 'BytesPerReq', 'BusyWorkers', 'IdleWorkers')


def poll_apache(poller_buffer, parsed_params, **kw):
    return kw['pobj'].tcp_client.get_tcp(
        kw['attribute'].host, 80,
        'GET /server-status?auto HTTP/1.1\r\nHost: {}\r\n\r\n'.format(
            kw['attribute'].host.mgmt_address), 1000, cb_apache, **kw)


def cb_apache(host, response, connect_time, error, pobj, attribute, **kwargs):
        ...
        if match is not None:
            ...
            for k, v in match:
                try:
                    ...
                    retvals[idx] = v
            pobj.poller_callback(attribute.id, retvals)
            return
