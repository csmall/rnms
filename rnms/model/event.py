...
class Event(DeclarativeBase):
    """
    An Event is something that has either happened to a host or to an
    Attribute.
    ...
    fields = relationship('EventField', backref='event',
                          order_by='EventField.tag',
                          cascade='all,delete,delete-orphan')

    def __init__(self, event_type, host=None, attribute=None, event_state=None,
                 field_list=None):
        self.event_type = event_type
        if attribute is not None:
            self.attribute = attribute
            self.host = attribute.host
            if event_type.generate_alarm is True:
                self.alarmed = True
        ...
        if field_list is not None:
            for field_tag in field_list.keys():
                self.fields.append(EventField(field_tag,
    ...
    def create_sla(cls, attribute, info, details):
        ...
        return Event(event_type=event_type, attribute=attribute,
                     field_list=event_fields, event_state=event_state)
    ...
    def create_admin(cls, host=None, attribute=None, info=None):
        ...
        if event_type is not None:
            return Event(event_type=event_type, host=host, attribute=attribute,
    ...
    def text(self):
        ...
        """
        return fill_fields(self.event_type.text, host=self.host,
                           attribute=self.attribute, event=self)
...
class EventState(DeclarativeBase):
    ...
    def __repr__(self):
        return '<EventState: %s (%s)>' % (
            self.display_name, self.internal_state)
