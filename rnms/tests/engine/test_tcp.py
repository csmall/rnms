...
class TestTCP(object):
    ...
    def test_noconnect(self):
        ...
        assert(self.tcp_client.get_tcp('172.16.242.250', 1234, '', 0, self.my_callback, obj=self))
        self.poll()
        self.assert_cb_error_code(errno.EHOSTUNREACH)

    def test_connrefused(self):
        """ Connection refused to bad ports """
        assert(self.tcp_client.get_tcp('127.0.0.1', 9999, 'HELO foo', 0, self.my_callback, obj=self))
        self.poll()
        self.assert_cb_error_code(errno.ECONNREFUSED)

    def test_okipv4(self):
        """ An open port gets data """
        assert(self.tcp_client.get_tcp('127.0.0.1', 25, 'HELO foo', 1, self.my_callback, obj=self))
        self.poll()
        self.assert_cb_error_code(None)

    def test_okipv6(self):
        """ IPv6 host connect """
        assert(self.tcp_client.get_tcp('::1', 25, 'HELO foo', 1, self.my_callback, obj=self))
        self.poll()
        self.assert_cb_error_code(None)

    def test_far_host(self):
        """ Far host has connect time """
        assert(self.tcp_client.get_tcp('127.0.0.1', 22, '', 1, self.my_callback, obj=self))
        self.poll()
        self.assert_cb_error_code(None)
