...
from rnms.lib.poller.plugins.snmp import poll_snmp_counter, cb_snmp_counter,\
    poll_snmp_status, cb_snmp_status, poll_snmp_walk_average,\
    cb_snmp_walk_average, poll_snmp_counter_mul
...
class TestSnmpPoller(PollerTest):
    ...
    def test_counter_poll_empty(self):
        ...
        eq_(poll_snmp_counter(self.poller_buffer, '',
                              **self.test_kwargs), False)

    def test_counter_poll_badparam(self):
        """ snmp_counter with bad params returns false """
        eq_(poll_snmp_counter(self.poller_buffer, 'fooo',
                              **self.test_kwargs), False)

    def test_counter_poll(self):
        """ snmp_counter works """
        eq_(poll_snmp_counter(self.poller_buffer, '1.2.3',
                              **self.test_kwargs), True)
    ...
    def test_counter_poll_mul_empty(self):
        ...
        eq_(poll_snmp_counter_mul(self.poller_buffer, '',
                                  **self.test_kwargs), False)

    def test_counter_poll_mul_badparam(self):
        """ snmp_counter_mul with bad params returns false """
        eq_(poll_snmp_counter_mul(self.poller_buffer, 'fooo',
                                  **self.test_kwargs), False)

    def test_counter_poll_mul(self):
        """ poll_snmp_counter_mull works """
        eq_(poll_snmp_counter_mul(self.poller_buffer, '1.2.3|10',
                                  **self.test_kwargs), True)
    ...
    def test_status_poll_empty(self):
        ...
        eq_(poll_snmp_status(self.poller_buffer, '',
                             **self.test_kwargs), False)

    def test_status_poll_badparam(self):
        """ snmp_status with bad params returns false """
        eq_(poll_snmp_status(self.poller_buffer, 'fooo',
                             **self.test_kwargs), False)

    def test_status_poll_badoid(self):
        """ snmp_status with bad oid returns false """
        eq_(poll_snmp_status(self.poller_buffer, 'badoid|1=2',
                             **self.test_kwargs), False)

    def test_status_poll_full(self):
        """ poll_snmp_status with mapping and default return true """
        eq_(poll_snmp_status(self.poller_buffer, '1.2.3|1=2|3',
                             **self.test_kwargs), True)
        self.assert_get_str_called(
            (1, 2, 3), cb_snmp_status,
            {'mapping': '1=2', 'default_state': '3'})

    def test_status_poll_no_default(self):
        """ poll_snmp_status with mapping return true """
        eq_(poll_snmp_status(self.poller_buffer, '1.2.3|1=2',
                             **self.test_kwargs), True)
        self.assert_get_str_called(
            (1, 2, 3), cb_snmp_status,
            {'mapping': '1=2', 'default_state': None})

    def test_status_poll_no_mapping(self):
        """ poll_snmp_status with no mapping return False """
        eq_(poll_snmp_status(self.poller_buffer, '1.2.3', **self.test_kwargs),
            False)
    ...
    def test_walkavg_poll_empty(self):
        ...
        eq_(poll_snmp_walk_average(self.poller_buffer, '', **self.test_kwargs),
            False)

    def test_walkavg_poll(self):
        """ poll_snmp_walk_average """
        eq_(poll_snmp_walk_average(self.poller_buffer, '1.2.3',
                                   **self.test_kwargs), True)
