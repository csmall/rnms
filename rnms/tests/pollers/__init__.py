...
class PollerTest(object):
    ...
    def set_attribute_fields(self, fields):
        ...
        self.test_attribute.get_fields =\
            mock.Mock(return_value=self.test_attribute_fields)
    ...
    def assert_callback(self, values):
        ...
        self.poller_cb.assert_called_once_with(self.test_attribute_id, values)
    ...
    def assert_get_int_called(self, cb_func, extra_kwargs=None,
                              oid=(1, 2, 3)):
        if extra_kwargs is not None:
            self.test_kwargs.update(extra_kwargs)
        self.snmp_engine.get_int.assert_called_once_with(
    ...
    def assert_tcp_client_called(self, port, send_str, max_bytes, cb_fun):
        ...
        self.tcp_client.get_tcp.assert_called_once_with(
            self.test_host, port, send_str, max_bytes, cb_fun,
            **self.test_kwargs)
